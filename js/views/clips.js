...
(function () {
  ...
  function captureSheet(meta, file, stored, why) {
    ...
    m = FB.ui.modal(`<h2>${stored ? '🎥 Clip saved' : '🎥 Clip not saved'}</h2>
      ...
      ${stored ? `<div class="field-row"><label>Label</label><input type="text" id="clabel" value="${U.esc(meta.label)}"></div>
                  <div class="field-row"><label>Comment</label><textarea id="cnotes" placeholder="What happened? Who made the play?" style="min-height:60px"></textarea></div>`
    ...
    const notes = q('#cnotes'); if (notes) notes.onchange = () => S.mutate(st => { const c = st.videos.find(v => v.id === meta.id); if (c) c.notes = notes.value.trim(); });
  ...
  /* ---------- label + comment ---------- */
  function editClip(c) {
    const m = FB.ui.modal(`<h2>Clip details</h2><form id="cf">
      <div class="field-row"><label>Label</label><input type="text" name="label" value="${U.esc(clipLabel(c))}"></div>
      <div class="field-row"><label>Comment</label><textarea name="notes" placeholder="What happened? Who made the play?">${U.esc(c.notes || '')}</textarea></div>
      <div class="small muted" style="margin-bottom:12px">${detail(c)}</div>
      <div class="actions"><button type="button" class="btn" id="cancel">Cancel</button><button type="submit" class="btn primary">Save</button></div></form>`, { focus: false });
    m.el.querySelector('#cancel').onclick = m.close;
    m.el.querySelector('#cf').onsubmit = e => {
      e.preventDefault();
      const d = FB.ui.formData(e.target);
      S.mutate(st => { const v = st.videos.find(x => x.id === c.id); if (v) { v.label = d.label || clipLabel(c); v.notes = d.notes || ''; } });
      m.close();
    };
  }
  ...
  function rowHTML(c) {
    return `<div class="card clip-row tap" data-clip="${c.id}" title="Tap to edit the label or comment">
      <div class="grow"><div class="card-title">${U.esc(clipLabel(c))}</div>
        ...
        ${c.notes ? `<div class="small clip-notes">💬 ${U.esc(c.notes)}</div>` : `<div class="small muted clip-notes">💬 <i>Add a comment</i></div>`}
  ...
  const FOOT = `<p class="small muted" style="text-align:center;margin-top:16px">Clips stay on this device and are not in the JSON backup. Use Save to Photos for a permanent copy.</p>`;
  ...
  /* #/clips → one card per game; #/clips?game=<id|none> → that game's clips */
  FB.views.clips = {
    render(root, route) {
      const st = S.get();
      const gid = (route.query && route.query.game) || '';
      if (gid) return renderGame(root, st, gid);
      FB.ui.setTitle(`<a class="btn ghost sm" href="#/games" aria-label="Back">‹</a><h1>Clips</h1>`);
      if (!st.videos.length) {
        root.innerHTML = `<div class="empty">No clips yet.<br><br><span class="small">Open a play in the <a href="#/playbook">playbook</a> and tap <b>🎥</b> to record one. When you stop, you land back on the playbook.</span></div>`;
        return;
      }
      ...
      for (const [k, list] of groups) {
        ...
        const commented = list.filter(c => c.notes).length;
        html += `<a class="card tap" href="#/clips?game=${k}" style="display:block;text-decoration:none">
          ...
          <div class="row wrap small muted" style="margin-top:6px;gap:12px"><span>${list.length} clip${list.length === 1 ? '' : 's'}</span>${ms ? `<span>${M.fmtDuration(ms)}</span>` : ''}<span>${M.fmtBytes(bytes)}</span>${commented ? `<span>💬 ${commented}</span>` : ''}</div></a>`;
      }
  ...
  function renderGame(root, st, gid) {
    ...
    FB.ui.setTitle(`<a class="btn ghost sm" href="#/clips" aria-label="All clips">‹</a><h1>${g ? U.esc(gameTitle(g)) + ' · clips' : 'Other clips'}</h1>${g ? `<a class="btn sm" href="#/games/${g.id}">Game</a>` : ''}`);
    const clips = st.videos.filter(c => groupKey(c) === gid).sort(bySeq);
    if (!clips.length) {
      root.innerHTML = `<div class="empty">No clips for this game yet.<br><br><span class="small">Pick this game in the playbook's lineup bar, open a play and tap <b>🎥</b>.</span></div>`;
      return;
    }
    ...
    root.querySelectorAll('[data-clip]').forEach(el => {
        ...
        el.onclick = e => { if (!e.target.closest('button, a')) editClip(c); };
