(function () {
  ...
  function gameCard(g) {
    ...
    return `<a class="card tap" href="#/games/${g.id}" style="display:block;text-decoration:none">
      ...
      <div class="row wrap small" style="margin-top:8px;gap:12px"><span>🍎 ${snack ? U.esc(snack) : '<span class="muted">no snack assigned</span>'}</span><span>${set ? '✅ rotation set' : '<span class="muted">rotation not set</span>'}</span></div></a>`;
  }
  ...
  FB.views.game = {
    render(root, route) {
      ...
      let html = `<div class="card">
        ...
        <a class="btn sm" href="#/gamesheet/${g.id}">🖨 Game sheet</a></div>
