...
(function () {
  ...
  function renderList(root, route) {
    ...
    root.innerHTML = html;
    bindLineupBar(root);
    // 🎥 Defense: same camera as the 🎥 in view mode (in-app recorder, or the iOS camera hand-off), tagged as a defensive snap
    const recDefIn = root.querySelector('#recDefIn');
    document.getElementById('recDef').onclick = () => { if (FB.recorder && FB.recorder.enabled()) location.hash = '#/record/defense'; else { recDefIn.value = ''; recDefIn.click(); } };
    ...
    recDefIn.addEventListener('cancel', () => { recDefIn.value = ''; });
